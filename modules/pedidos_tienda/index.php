...
$dash = $db->query("
    SELECT
        SUM(CASE WHEN estado NOT IN ('entregado','cancelado','despachado') THEN 1 ELSE 0 END)                                                                          AS pipeline_activo,
        SUM(CASE WHEN DATE_FORMAT(fecha_compra,'%Y-%m') = DATE_FORMAT(CURDATE(),'%Y-%m') AND estado != 'cancelado' THEN COALESCE(woo_total,0) ELSE 0 END)              AS ingresos_mes,
        SUM(CASE WHEN YEAR(fecha_compra) = YEAR(CURDATE()) AND estado != 'cancelado' THEN COALESCE(woo_total,0) ELSE 0 END)                                            AS ingresos_anio,
        SUM(CASE WHEN DATE_FORMAT(fecha_compra,'%Y-%m') = DATE_FORMAT(CURDATE(),'%Y-%m') AND estado != 'cancelado' THEN 1 ELSE 0 END)                                  AS pedidos_mes,
        SUM(CASE WHEN estado = 'listo_envio' THEN 1 ELSE 0 END)                                                                                                       AS cnt_listo_envio,
        ...
        SUM(CASE WHEN YEAR(fecha_compra) = YEAR(CURDATE()) AND estado = 'entregado' THEN 1 ELSE 0 END)                                                                 AS entregados_anio,
        SUM(CASE WHEN YEAR(fecha_compra) = YEAR(CURDATE()) AND estado != 'cancelado' THEN 1 ELSE 0 END)                                                                AS pedidos_anio
    FROM tienda_pedidos
")->fetch();

$top_colegios = $db->query("
    SELECT COALESCE(col.nombre, p.colegio_nombre) AS nombre,
           COUNT(*)                               AS pedidos,
           SUM(COALESCE(p.woo_total,0))           AS total_cop
    FROM tienda_pedidos p
    LEFT JOIN colegios col ON col.id = p.colegio_id
    WHERE p.estado != 'cancelado'
      AND YEAR(p.fecha_compra) = YEAR(CURDATE())
      AND COALESCE(col.nombre, p.colegio_nombre) IS NOT NULL
      AND COALESCE(col.nombre, p.colegio_nombre) != ''
    GROUP BY COALESCE(col.nombre, p.colegio_nombre)
...
/* ── Dashboard ── */
.dash-grid{display:grid;gap:.85rem;margin-bottom:1rem}
.dash-kpis{grid-template-columns:repeat(4,1fr)}
.dash-mid{grid-template-columns:1fr}
.dash-bottom{grid-template-columns:1fr 1fr}
@media(max-width:900px){.dash-kpis{grid-template-columns:repeat(2,1fr)}.dash-bottom{grid-template-columns:1fr}}
@media(max-width:560px){.dash-kpis{grid-template-columns:1fr}}
.kpi-card{background:#fff;border:1px solid var(--rs-gray-200);border-radius:var(--rs-radius);padding:1rem 1.1rem;display:flex;flex-direction:column;gap:.25rem;transition:.15s}
.kpi-card:hover{box-shadow:0 4px 16px rgba(0,0,0,.07)}
.kpi-icon{width:34px;height:34px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1rem;margin-bottom:.2rem}
.kpi-val{font-size:1.55rem;font-weight:700;line-height:1.1;color:#0f172a}
.kpi-val.sm{font-size:1.2rem}
.kpi-lbl{font-size:.72rem;font-weight:600;color:var(--rs-text-muted);text-transform:uppercase;letter-spacing:.04em}
.kpi-sub{font-size:.72rem;color:var(--rs-text-muted);margin-top:.1rem}
.kpi-urgent .kpi-val{color:#dc2626}
.kpi-urgent{border-left:3px solid #dc2626}
.kpi-urgent.zero{border-left-color:var(--rs-gray-200)}
/* Pipeline */
.pipeline-card{background:#fff;border:1px solid var(--rs-gray-200);border-radius:var(--rs-radius);padding:.85rem 1.1rem}
.pipeline-title{font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--rs-text-muted);margin-bottom:.7rem}
.pipeline-flow{display:flex;align-items:stretch;gap:0;overflow-x:auto;padding-bottom:.1rem}
.pipe-step{flex:1;min-width:72px;display:flex;flex-direction:column;align-items:center;text-align:center;position:relative;text-decoration:none;padding:.55rem .3rem .5rem;border-radius:8px;transition:.12s;border:1.5px solid transparent}
.pipe-step:hover{background:var(--rs-gray-100);border-color:var(--rs-gray-200)}
.pipe-step.activo{border-color:currentColor}
.pipe-count{font-size:1.35rem;font-weight:800;line-height:1}
.pipe-lbl{font-size:.62rem;font-weight:600;color:var(--rs-text-muted);margin-top:.25rem;line-height:1.2;text-transform:uppercase;letter-spacing:.03em}
.pipe-arrow{color:var(--rs-gray-300);font-size:.9rem;display:flex;align-items:center;padding:0 .1rem;flex-shrink:0;align-self:center;margin-top:-.6rem}
/* Bottom panels */
.dash-panel{background:#fff;border:1px solid var(--rs-gray-200);border-radius:var(--rs-radius);padding:.85rem 1.1rem}
.panel-title{font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--rs-text-muted);margin-bottom:.7rem}
.urgency-row{display:flex;align-items:center;gap:.6rem;margin-bottom:.45rem}
.urgency-bar-wrap{flex:1;height:7px;background:var(--rs-gray-100);border-radius:4px;overflow:hidden}
.urgency-bar{height:100%;border-radius:4px;transition:width .4s}
.urgency-cnt{font-size:.82rem;font-weight:700;min-width:24px;text-align:right}
.urgency-lbl{font-size:.73rem;color:var(--rs-text-muted);min-width:60px}
.top-col-row{display:flex;align-items:center;gap:.5rem;padding:.32rem 0;border-bottom:1px solid var(--rs-gray-100)}
.top-col-row:last-child{border-bottom:none}
.top-col-rank{width:18px;height:18px;border-radius:50%;background:var(--rs-gray-100);color:var(--rs-text-muted);font-size:.65rem;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.top-col-rank.gold{background:#fef9c3;color:#854d0e}
.top-col-name{flex:1;font-size:.78rem;font-weight:600;color:#1e293b;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.top-col-val{font-size:.75rem;font-weight:700;color:#16a34a;white-space:nowrap}
.top-col-peds{font-size:.65rem;color:var(--rs-text-muted)}
</style>
...
<!-- ── Dashboard ── -->
<?php
$activoTotal  = max(1, (int)$stats['rojos'] + (int)$stats['amarillos'] + (int)$stats['verdes']);
$pctRojo      = round(($stats['rojos']     / $activoTotal) * 100);
$pctAmarillo  = round(($stats['amarillos'] / $activoTotal) * 100);
$pctVerde     = round(($stats['verdes']    / $activoTotal) * 100);
$tasa_entrega = $dash['pedidos_anio'] > 0 ? round(($dash['entregados_anio'] / $dash['pedidos_anio']) * 100) : 0;

$pipe_estados = [
    'pendiente'        => ['lbl'=>'Pendiente',    'color'=>'#854d0e'],
    'aprobado'         => ['lbl'=>'Aprobado',      'color'=>'#1d4ed8'],
    'en_produccion'    => ['lbl'=>'En Prod.',      'color'=>'#9a3412'],
    'listo_produccion' => ['lbl'=>'Listo Prod.',   'color'=>'#065f46'],
    'en_alistamiento'  => ['lbl'=>'Alistando',     'color'=>'#5b21b6'],
    'listo_envio'      => ['lbl'=>'Listo Envío',   'color'=>'#155e75'],
    'despachado'       => ['lbl'=>'Despachado',    'color'=>'#14532d'],
    'entregado'        => ['lbl'=>'Entregado',     'color'=>'#475569'],
];
?>
<div class="dash-grid dash-kpis no-print">
  <!-- KPI 1: Pipeline activo -->
  <div class="kpi-card">
    <div class="kpi-icon" style="background:#dbeafe;color:#1d4ed8"><i class="bi bi-activity"></i></div>
    <div class="kpi-val"><?= number_format($dash['pipeline_activo'],0,',','.') ?></div>
    <div class="kpi-lbl">Pipeline activo</div>
    <div class="kpi-sub"><?= $dash['pedidos_anio'] ?> pedidos recibidos este año</div>
  </div>
  <!-- KPI 2: Urgentes -->
  <div class="kpi-card kpi-urgent <?= (int)$stats['rojos'] === 0 ? 'zero' : '' ?>">
    <div class="kpi-icon" style="background:#fee2e2;color:#dc2626"><i class="bi bi-exclamation-triangle-fill"></i></div>
    <div class="kpi-val"><?= (int)$stats['rojos'] ?></div>
    <div class="kpi-lbl">Urgentes (&gt;7 días)</div>
    <div class="kpi-sub"><?= (int)$stats['cnt_pendiente'] ?> pendientes de aprobación</div>
  </div>
  <!-- KPI 3: Ingresos del mes -->
  <div class="kpi-card">
    <div class="kpi-icon" style="background:#dcfce7;color:#16a34a"><i class="bi bi-currency-dollar"></i></div>
    <div class="kpi-val sm">$<?= number_format((float)$dash['ingresos_mes'],0,',','.') ?></div>
    <div class="kpi-lbl">Ingresos del mes</div>
    <div class="kpi-sub"><?= (int)$dash['pedidos_mes'] ?> pedidos · año: $<?= number_format((float)$dash['ingresos_anio']/1000000,1,',','.') ?>M</div>
  </div>
  <!-- KPI 4: Tasa de entrega -->
  <div class="kpi-card">
    <div class="kpi-icon" style="background:#faf5ff;color:#7c3aed"><i class="bi bi-patch-check-fill"></i></div>
    <div class="kpi-val"><?= $tasa_entrega ?>%</div>
    <div class="kpi-lbl">Tasa de entrega (año)</div>
    <div class="kpi-sub"><?= $dash['dias_promedio'] ? $dash['dias_promedio'].' días promedio' : 'Sin datos de entrega' ?></div>
  </div>
</div>

<!-- Pipeline -->
<div class="pipeline-card no-print" style="margin-bottom:.85rem">
  <div class="pipeline-title"><i class="bi bi-diagram-3 me-1"></i>Flujo de pedidos</div>
  <div class="pipeline-flow">
    <?php foreach ($pipe_estados as $key => $info):
      $cnt = (int)($stats['cnt_' . $key] ?? 0);
      $url = 'index.php?' . http_build_query(array_merge($_GET, ['estado' => $key, 'pag' => 1]));
    ?>
    <a href="<?= htmlspecialchars($url) ?>" class="pipe-step <?= $fEstado === $key ? 'activo' : '' ?>"
       style="color:<?= $info['color'] ?>">
      <div class="pipe-count" style="color:<?= $cnt > 0 ? $info['color'] : '#cbd5e1' ?>"><?= $cnt ?></div>
      <div class="pipe-lbl"><?= $info['lbl'] ?></div>
    </a>
    <?php if ($key !== 'entregado'): ?>
...
<!-- Semáforo + Top colegios -->
<div class="dash-grid dash-bottom no-print" style="margin-bottom:1rem">
  <!-- Urgencia -->
  <div class="dash-panel">
    <div class="panel-title"><i class="bi bi-traffic-light me-1"></i>Urgencia del pipeline</div>
    <div class="urgency-row">
      <span class="urgency-lbl" style="color:#dc2626">🔴 Urgente</span>
      <div class="urgency-bar-wrap"><div class="urgency-bar" style="width:<?= $pctRojo ?>%;background:#ef4444"></div></div>
      <a href="index.php?prioridad=rojo" class="urgency-cnt" style="color:#dc2626"><?= (int)$stats['rojos'] ?></a>
    </div>
    <div class="urgency-row">
      <span class="urgency-lbl" style="color:#b45309">🟡 Normal</span>
      <div class="urgency-bar-wrap"><div class="urgency-bar" style="width:<?= $pctAmarillo ?>%;background:#f59e0b"></div></div>
      <a href="index.php?prioridad=amarillo" class="urgency-cnt" style="color:#b45309"><?= (int)$stats['amarillos'] ?></a>
    </div>
    <div class="urgency-row">
      <span class="urgency-lbl" style="color:#16a34a">🟢 En tiempo</span>
      <div class="urgency-bar-wrap"><div class="urgency-bar" style="width:<?= $pctVerde ?>%;background:#22c55e"></div></div>
      <a href="index.php?prioridad=verde" class="urgency-cnt" style="color:#16a34a"><?= (int)$stats['verdes'] ?></a>
    </div>
    <div class="urgency-row" style="margin-top:.3rem">
      <span class="urgency-lbl" style="color:#64748b">✅ Compl.</span>
      <div class="urgency-bar-wrap">
        <?php $pctComp = $stats['total'] > 0 ? round((((int)$stats['cnt_entregado']+(int)$stats['cnt_despachado']+(int)$stats['cnt_cancelado'])/$stats['total'])*100) : 0; ?>
        <div class="urgency-bar" style="width:<?= $pctComp ?>%;background:#94a3b8"></div>
      </div>
      <a href="index.php?prioridad=completado" class="urgency-cnt" style="color:#64748b"><?= (int)$stats['cnt_entregado']+(int)$stats['cnt_despachado']+(int)$stats['cnt_cancelado'] ?></a>
    </div>
  </div>

  <!-- Top colegios -->
  <div class="dash-panel">
    <div class="panel-title"><i class="bi bi-building me-1"></i>Top colegios — <?= date('Y') ?></div>
    <?php if (empty($top_colegios)): ?>
      <p class="text-muted small mb-0">Sin datos de colegios este año.</p>
    <?php else:
      $max_top = max(array_column($top_colegios, 'total_cop')) ?: 1;
      foreach ($top_colegios as $i => $c): ?>
    <div class="top-col-row">
      <div class="top-col-rank <?= $i === 0 ? 'gold' : '' ?>"><?= $i+1 ?></div>
      <div style="flex:1;min-width:0">
        <div class="top-col-name" title="<?= htmlspecialchars($c['nombre']) ?>"><?= htmlspecialchars($c['nombre']) ?></div>
        <div style="height:3px;background:var(--rs-gray-100);border-radius:2px;margin-top:3px;overflow:hidden">
          <div style="height:100%;width:<?= round(($c['total_cop']/$max_top)*100) ?>%;background:#16a34a;border-radius:2px"></div>
        </div>
      </div>
      <div style="text-align:right">
        <div class="top-col-val">$<?= number_format($c['total_cop'],0,',','.') ?></div>
        <div class="top-col-peds"><?= $c['pedidos'] ?> ped.</div>
      </div>
    </div>
